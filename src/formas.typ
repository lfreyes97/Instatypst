...
//
//  Por qué: un contorno COMPUESTO (rect redondeado + cola de bocadillo
//  en un solo trazo, sin costura) es trivial en SVG (<path d="..."> con
//  arcos `A` para las esquinas y líneas `L` para la cola) y bastante más
//  trabajoso con curve() nativo de Typst — el primer intento fue
//  box(fondo) + curve(cola) superpuestos a mano, con un margen de error
//  (`altura - 1.5pt`) para tapar la costura entre las dos piezas. Con
//  SVG nunca hay dos piezas: un solo `<path>` es el contorno entero.
//
...
//  concatenando con saltos de línea de por medio.
//
...
//  contorno compuesto como este. Cuando se retome, esas variantes van
//  como nuevas funciones `_svg-*`/`forma-*` en este archivo (probablemente
//  `forma-bocadillo(..., gradiente: (color, color))`, no parámetros sueltos
//  encima de la firma actual).
// ─────────────────────────────────────────────────────────────
...
// Lego: la FORMA sola (sin texto encima), lista para usar como fondo de
// cualquier bloque — mismo espíritu que bq-frame() en blockquotes.typ.
// Devuelve contenido ya dimensionado a (w, h + tail-h); quien la use
// pone su propio texto encima con place(top + left) sobre ese mismo
// ancho, dentro de un box() que envuelva a ambos (ver nota de place() en
// el prototipo — dos place() sueltos, uno tras otro sin un contenedor en
// común, NO comparten origen).
//
// `lado:` "izq" | "der" | "centro" — posición de la cola.
#let forma-bocadillo(w: 210pt, h: 140pt, r: 18pt, lado: "izq", tail-w: 28pt, tail-h: 22pt, fill: black) = {
  let w-n = w / 1pt
  let h-n = h / 1pt
  let r-n = r / 1pt
  let tw-n = tail-w / 1pt
